...
\begin{itemize}
\item
(Free, open-source) \IT{ent}\footnote{\url{http://www.fourmilab.ch/random/}}: Ferramenta para análise de entropia.
Leia mais sobre entropia: \myref{entropy}.

\item
\label{Hiew}
\IT{Hiew}\footnote{\href{http://go.yurichev.com/17035}{hiew.ru}}:
para pequenas modificações de código em arquivos binários.
Has dssembler/issassember.

\item (Free, open-source) \IT{GHex}\footnote{\url{https://wiki.gnome.org/Apps/Ghex}}: Editor hexadecimal para Linux.
...
Sysinternals alternativa\footnote{\url{https://technet.microsoft.com/en-us/sysinternals/strings}}
supporting wide char strings (UTF-16, largamente utilizada no Windows).

\item (Free, open-source) \IT{Binwalk}\footnote{\url{http://binwalk.org/}}: Análise de imagens de firmware.
...
\section{Está faltando alguma?}

Se você conhece alguma outra ferramenta que não está nesta lista, favor contactar-me\\
\TT{\EMAIL}.
