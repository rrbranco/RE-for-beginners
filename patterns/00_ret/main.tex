...
\PTBR{A função mais simples é indiscutivelmente uma que retorna um valor constante:}

\RU{Вот, например}\EN{Here it is}\PTBR{Vejamos}:
...
\RU{И вот что делает оптимизирующий GCC}\EN{Here's what both the optimizing GCC and MSVC compilers produce on the x86 platform}\PTBR{Aqui está o código que ambos, o GCC otimizado e compiladores MSVC, produzem na plataforma x86 }:
...
\PTBR{Há apenas duas intruções: a primeira coloca o valor 123 dentro do registrador \EAX, que por convencao e utilizado pra armazenar o valor de retorno e a segunda instrução é o \RET, que retorna a execução pro \gls{caller}.}
\RU{Вызывающая функция возьмет результат из регистра \EAX.}
\EN{The caller will take the result from the \EAX register.}
\PTBR{A chamada pegara o resultado do registrador \EAX.}
...
%In portuguese misleading can give the notion of Falso Cognato, should we use?
\EN{Vale notar que a instrução \MOV é um da a impressão errada em ambas as plataformas x86 e ARM \ac{ISA}s. }
\RU{На самом деле, данные не \IT{перемещаются}, а скорее \IT{копируются}.}
\EN{The data is not in fact \IT{moved}, but \IT{copied}.}
\PTBR{O dado não é \IT{movido} de fato, mas sim \IT{copiado}.}
