...
\PTBR{Vamos utilizar um exemplo famoso do livro}
...
\PTBR{A principal diferença entre códigos x86/ARM e x64/ARM64 é que os ponteiros para string tem 64-bits de tamanho.}
...
\PTBR{De fato, \ac{CPU}s modernas são 64-bit devido a redução do custo de memória e o aumento da demanda gerado por aplicações modernas. }
\RU{Поэтому все указатели теперь 64-битные.}\EN{As such, all pointers are now 64-bit.}
\PTBR{Como tal, todos os ponteiros agora são de 64 bits.}
% sections
